...
EXCLUDED_PATHS = []  # Paths to exclude from scraping
...
class DocsScraper:
    ...
    async def fetch_sitemap(self) -> List[str]:
        ...
        async with self.session.get(self.sitemap_url) as response:
            ...
            logger.info(f"Sitemap content: {content[:500]}")  # Log a portion of the sitemap to check its structure
            ...
            return [
                url for url in urls
                if url.startswith(self.base_url) and not self.should_exclude_url(url)
            ]
