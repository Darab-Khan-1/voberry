...
class RAGEnrichedAgent(Agent):
    ...
    async def livekit_docs_search(self, query: str) -> str:  # Changed signature
        f"""Search knowledge base for information using the given query.
        
        Args:
            query: The search query to look up in open weather documentation
        """
        try:
    ...
    async def on_message(self, message: ChatMessage):
        ...
        if "Content:" in search_result:
            ...
            await self.session.say(f"Sorry, I can't answer that. My knowledge is limited to open weather documentation.")
            # await super().on_message(message)

    async def on_enter(self):
        if self._annoy_index:
            greeting = f"Hi! I can help with open weather questions. What would you like to know?"
        else:
